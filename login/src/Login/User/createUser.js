...
class createUsers extends Component
{
    ...
    get_S4T03() {
        ...
        axiosCustom.get(serverUrl + 'api/auth/Users', option).then((response) => {
    //alert();
    ...
    OnBlurConfirmPassword(e) {
        if (e.target.value != "") {
            ...
            if (pass !== confirmPassword) {
                $('#ConfirmPasswordAlert').text("رمز عبور مطابقت ندارد");
                this.setState({ inValidConfirmPass: true })
            } else {
                $('#ConfirmPasswordAlert').empty();
                this.setState({ inValidConfirmPass: false })
            }
        }
    }
    //**=================================================================== *//
    OnBlurPassword(e) {
        if (e.target.value.length < 6) {
            $('#passAlert').text("رمز عبور نباید کمتر از 6 کاراکتر باشد");
    ...
    post_S4T01()
    {
        ...
        model.pass=this.state.pass;
        model.role=this.state.value_S4T01.name;
        axiosCustom.post(serverUrl +'api/auth/SendEmail', { name:model.name,email:model.email }).then((response) => {
           // this.refresh();
        ...
        axiosCustom.post(serverUrl +'api/auth/Register', { fullName:model.name,userName: model.name,email:model.email ,passWord:model.pass,role:model.role }).then((response) => {
            this.refresh();
            this.setState({ onEndRequest: true });
...
{/* <PasswordField
                ...
                <br />
SS
                <GridUsers onEndRequest={this.state.onEndRequest} list={this.state.list_grid} onSelectItem={this.get_SelectedItem} gridDatastate={this.state.gridDatastate} onDatastateChange={this.onDatastateChange} />
